...
class StepController extends React.Component {
  ...
  disableNext(currentStep, profile) {
    ...
    switch (currentStep) {
      case ALL_STEPS.careerInterests:
        return careerInterests.length === 0;
      case ALL_STEPS.bio:
        return skills.length === 0 || bio.length === 0;
      case ALL_STEPS.uploadResume:
        return resumeFile.fullPath.length === 0;
      case ALL_STEPS.education:
        return education.length === 0;
      case ALL_STEPS.experience:
        return experience.length === 0;
      case ALL_STEPS.profileDetails:
        return currentUniversity.length === 0 || skills.length === 0;
      case ALL_STEPS.other:
        return workingRights.length === 0; // || phoneNumber.length !== 10;
      default:
        return false;
    }
  }
  ...
  showSave(currentStep, profile) {
    switch (currentStep) {
      ...
      case ALL_STEPS.profileDetails:
        return false;
      case ALL_STEPS.other:
        return false; // || phoneNumber.length !== 10;
      default:
        return false;
    }
  }
  ...
  render() {
    ...
    const saveLink = (
      <StyledLink
        onClick={() => {
          this.handleSave(currentStep);
        }}
        className={classes.finishLabel}
        id={`saveAt-${currentStep}`}
        key={`saveAt-${currentStep}`}
      >
        Finish Later
      </StyledLink>
    );
    const saveButtonWithConditions =
      this.showSave(currentStep, profile) && !isLoading && saveLink;
    const webButtons = (
    ...
    return (
      <div>
        {isMobile && (
          <DotMobileStepper
            nextDisabler={checkComplition(currentStep, profile)}
            handleNext={() => {
              this.handleNext(currentStep);
            }}
            handleBack={() => {
        ...
        <Grid container direction="column" justify="space-between">
          ...
          {isMobile && (
            <Grid
              ...
              justify="flex-end"
            >
              <Grid item>{saveButtonWithConditions}</Grid>
            </Grid>
          )}
        </Grid>
      </div>
...
export default withRouter(
  withStyles(styles, { withTheme: true })(StepController)
);

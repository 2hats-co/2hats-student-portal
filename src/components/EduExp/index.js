...
class EducationContainer extends React.Component {
  ...
  handleDeleteDialog(key, item) {
    this.setState({
      deleteDialog: {
        key: key,
        heading: item.degree || item.title,
        subheading: item.university || item.organisation,
      },
    });
  }
  ...
  render() {
      ...
      const datedData = data.map(x => {
        if (x.endDate === 'Present') {
          return Object.assign(
            { endMoment: moment('Dec 2100', 'MMM YYYY').valueOf() },
            x
          );
        } else {
          return Object.assign(
            { endMoment: moment(x.endDate, 'MMM YYYY').valueOf() },
            x
          );
        }
      });
    ...
    return (
      <div>
        <Grid
          style={{ width: '100%', maxWidth: this.props.width }}
          container
          direction="column"
          alignItems="center"
        >

...
class EducationContainer extends React.Component {
  ...
  handleEditDialog(key,item){
    ...
    const prefilledForm = _.map(emptyForm,(field)=>Object.assign(field,{value:item[field.name]}))
    console.log(prefilledForm)
  this.setState({dialog:{isOpen:true,key:key,fields:prefilledForm}})
      ...
      let items 
        ...
        items = _.map(Object.values(this.props.profile)[0][name],(item,key)=>{
          if(item){return(<EduExpCard
            ...
            editHandler={()=>{this.handleEditDialog(key,item)}}
            deleteHandler={()=>{this.handleDelete(key)}}
          />)
        }}
...
EducationContainer.protoTypes = {
  ...
  const enhance = compose(
    ...
    withHandlers({
      loadData: props => listenerSettings => props.firestore.setListener(listenerSettings),
      onCreate: props => (item,subCollection) =>
        props.firestore.add({ collection: 'profiles', doc: props.uid,subcollections:[{collection:subCollection}] }, {
          ...item,
          createdAt: props.firestore.FieldValue.serverTimestamp()
        }),
        onUpdate: props => (key,item,subCollection) =>
        props.firestore.set({ collection: 'profiles', doc: props.uid,subcollections:[{collection:subCollection, doc:key}] }, {
          ...item,
          createdAt: props.firestore.FieldValue.serverTimestamp()
        }),
      onDelete: props => (itemKey,subCollection) => props.firestore.delete({ collection: `profiles/${props.uid}/${subCollection}`, doc: itemKey})
    // onUpdate
    }),
    ...
    lifecycle({
      ...
      componentWillMount() {
        console.log('firestore mounting',this.props)
        const {name } = this.props
        const listenerSettings = { collection: 'profiles', doc: this.props.uid, subcollections:[{ collection: name }]}
  ...
  export default 
  enhance(
      withStyles(styles)(EducationContainer)
    )

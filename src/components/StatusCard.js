...
import red from "sp2-material-ui/core/colors/red";
import green from "sp2-material-ui/core/colors/green";
...
class StatusCard extends React.Component {
  ...
  getConfirmationDialog(dialog) {
    switch (dialog) {
      case "submit":
        return {
          ...
          checkbox: {
            isChecked: false,
            label:
              "I understand I can not submit again until my resume has been reviewed."
          },
          confirm: {
            label: "Submit",
            action: () => {
      ...
      case "build":
        return {
          title: "Use Our Resume Builder Instead",
          body: [
            "Are you sure you want to build a new resume instead?",
            "All your previous progress will be saved, and you can always choose to upload an existing resume in the future."
          ],
          confirm: {
            label: "Build",
            action: () => {
      ...
      case "upload":
        return {
          title: "Upload Existing Resume Instead",
          body: [
            `Are you sure you want to switch to uploading your resume instead?`,
  ...
  getStatusPrompt(profile) {
    const uploadLink = {
      label: "Upload Existing Resume Instead",
      action: () => {
        this.setState({
          confirmationDialog: this.getConfirmationDialog("upload")
        });
      }
    };
    const uploadButton = {
      label: "Upload Resume",
      action: () => {
        this.props.goTo(`${routes.UPLOAD_RESUME}?step=3`);
      }
    };
    const pendingButton = {
    ...
    const buildLink = {
      label: "Use Our Resume Builder Instead",
      action: () => {
        this.setState({
          confirmationDialog: this.getConfirmationDialog("build")
        });
      }
    };
    const buildButton = {
    ...
    const submitButton = {
      ...
      action: () => {
        this.setState({
          confirmationDialog: this.getConfirmationDialog("submit")
        });
      }
    };
    const inCompleteBuildMessage =
      "It looks like you haven’t finished building your resume yet.";
    const inCompleteUploadMessage =
      "It looks like you haven’t filled out all the necessary information yet.";
    const noUploadMessage =
      "It looks like you haven’t uploaded your resume yet.";
    const completeMessage =
      "If you have completed your profile, click the Submit Profile button.";
    const inReviewMessage =
      "Your profile is currently under review. We will notify you regarding the feedback.";
    const chanceMessage =
      "Feedback received! Please review it and update your profile before submitting again.";
    ...
    const helpIcon = (
      <HelpIcon className={this.props.classes.help} color="primary" />
    );
    ...
    return (
      <div>
        <Card
          ...
          style={
            hideToaster ? { backgroundColor: "#FFF4ED" } : { display: "none" }
          }
          elevation={0}
        >
          <Grid
                ...
                >
                      ...
                      return (
                        <Grid item xs sm="auto" style={{ marginRight: 16 }}>
                          ...
                          >
                            ...
                            >
                              <div style={{ marginTop: 0 }}> {x.label}</div>{" "}
                              {!x.disabled && (
                                <AnimateIcon>
                                  {" "}
                                  <ArrowIcon
                                    style={{ marginRight: -18, marginTop: -1 }}
                                  />
                                  <ArrowIcon
                  ...
                  {link && (
                    <Grid item style={{ marginRight: 8 }}>
                      <PopupSelector
                        items={[{ label: link.label, action: link.action }]}
                      />
                    </Grid>
                  )}
                </Grid>
        ...
        {this.state.confirmationDialog && (
          <ConfirmationDialog
            dialog={this.state.confirmationDialog}
            closeHandler={() => {
              this.setState({ confirmationDialog: null });
            }}
            checkHandler={() => {
              let newObj = Object.assign(this.state.confirmationDialog, {
                checkbox: {
                  isChecked: !this.state.confirmationDialog.checkbox.isChecked,

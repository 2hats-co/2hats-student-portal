...
export function checkComplition(currentStep, profile) {
  ...
  switch (currentStep) {
    case ALL_STEPS.careerInterests:
      return !careerInterests || careerInterests.value.length === 0;
    case ALL_STEPS.bio:
      return !bio || bio.length === 0;
    case ALL_STEPS.skills:
      return !skills || skills.length === 0;
    case ALL_STEPS.uploadResume:
      return !resumeFile || resumeFile.downloadURL.length === 0;
    case ALL_STEPS.education:
      return !education || education.length === 0;
    case ALL_STEPS.experience:
      return !experience || experience.length === 0;
    case ALL_STEPS.currentUniversity:
      return !currentUniversity || currentUniversity.length === 0;
    case ALL_STEPS.other:
      return !workingRights || workingRights.length === 0; // || phoneNumber.length !== 10;
    default:
      return false;
  }
}
export function isComplete(profile) {
  if (profile) {
    return STEP_LABELS[profile.process]
      .map(step => {

...
export const blobAvatarUploader=(intialBlob,callback)=>{
	...
	img.onload = function(){
		...
		compressor(intialBlob.type,minimumDimension,intialSize,
			(blob)=>{
...
function compressor(fileType,minimumDimension,intialSize,callback){
	const percentage = reductionPercentage(minimumDimension,smallest(intialSize.width,intialSize.height))

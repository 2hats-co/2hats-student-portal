...
export const imageCompressor=(blobURL,minimumDimension,callback)=>{
...
img.onload = function(){
	...
	var resize_size = reductionPercentage(minimumDimension,smallest(img.width,img.height)); //1-100
	//resize
	resize(resize_size,callback);
};
...
function resize(percentages, callback) {
	if(current_size == percentages){
...
function Compressor_class() {
	...
	/**
	 * Compressor resize. Detect cpu count and use best option for user.
	 * 
	...
	/**
	 * Compressor resize. Resize actual image.
	 * 
	...
	/**
	 * Compressor resize, multicore version - fast image resize/resample using Compressor filter.
	 * 
	 * @param {HtmlElement} canvas
	 * @param {int} width
	 * @param {int} height
	...
	/**
	 * Compressor resize - fast image resize/resample using Compressor filter. 1 cpu version!
	 * 
	 * @param {HtmlElement} canvas
	 * @param {int} width
	 * @param {int} height
